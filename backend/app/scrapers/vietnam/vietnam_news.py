...
class VietnamNewsScraper(BaseScraper):
    SOURCE = "vietnam_news"
    ...
    async def fetch_article_urls(self) -> list[str]:
        ...
        for section in self.SECTIONS:
            ...
            for a in soup.select("a[href]"):
                href = a.get("href", "")
                if re.search(r"/\d+/", href) and ".html" in href:
    ...
    def parse_article(self, url: str, html: str) -> RawArticle | None:
        ...
        pub_date = None
        date_el = soup.select_one("span.article-date") or soup.select_one("time")
        if date_el:
            date_text = date_el.get("datetime", "") or date_el.get_text(strip=True)
            for fmt in ("%Y-%m-%dT%H:%M:%S", "%B %d, %Y", "%d/%m/%Y"):
                try:
